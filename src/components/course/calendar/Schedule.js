...
class Schedule extends React.Component {
  ...
  render() {
    ...
    var sort_events = events.sort((a, b) => {
      return sorted_list.indexOf(a.dayOfWeek) > sorted_list.indexOf(b.dayOfWeek)
        ? 1
        : -1;
    });

    console.log(this.state.showEditSchedule);
    return (
      <div>
        <NavBarCentered>
          <ActiveSessionAlert />
          <TitleHeader
            ...
            title={`${this.props.course?.code} Schedule`}
          ...
          {this.state.showEventCard ? (
            events.length == 0 ? (
              <Card type="flex" justify="center" align="middle">
                ...
                <h3 style={{ margin: "15px" }}>
                  To add a session, press Edit Day
                </h3>
              </Card>
            ) : (
              <Carousel
            ...
            <EditSchedule dayOfWeek={this.state.editDayOf} />
          )}
        </NavBarCentered>
      </div>
    );
    ...
    sessions: [
      {
        startTime: "3:30PM",
        endTime: "4:30PM",
        assistants: ["Brad Pitt ", "Beth Carrington"],
      },
      {
        startTime: "5:30PM",
        endTime: "6:30PM",
        assistants: ["Jasmine Harris"],
      },
    ],
  },
