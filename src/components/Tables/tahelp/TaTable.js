...
const createTag = (stage) => {
  if (stage === "Just Started") {
    return <Tag style={{borderRadius: "20px"}} color="green" key={stage}>{`${stage}`}</Tag>;
  } else if (stage === "Debugging Solution") {
    return <Tag style={{borderRadius: "20px"}} color="volcano" key={stage}>{`${stage}`}</Tag>;
  } else {
    return <Tag style={{borderRadius: "20px"}} color="purple" key={stage}>{`${stage}`}</Tag>;
  }
};
